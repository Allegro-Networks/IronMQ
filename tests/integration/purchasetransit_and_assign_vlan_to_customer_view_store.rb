...
class TestTransit < Test::Unit::TestCase
  ...
  def test_purchase_transit_assigns_vlan_view_store
    ...
    transit_vlan_tags_view_store = mongo_factory.create('transit-vlantags', VIEW_STORE)
    ...
    transit_vlan_tags_view_store.save(vlan)
    ...
    vlans = transit_vlan_tags_view_store.find("service_reference" => service_reference)
    
    assigned_vlan_in_view_store = vlans.to_a[0]

    assert_equal(vlans.count, 1)

    assert_equal(expected_assigned_vlan_tag, assigned_vlan_in_view_store["VlanTag"])
    assert_equal(service_reference, assigned_vlan_in_view_store["service_reference"])
    assert_equal(service_reference, assigned_vlan_in_view_store["_id"])
    assert_equal(saved_service["customer_asn"], assigned_vlan_in_view_store["ASN"])
  end
