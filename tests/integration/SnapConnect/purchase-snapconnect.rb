...
class TestSnapConnectWithNoVLANTag < Test::Unit::TestCase
  ...
  def test_purchase_snapconnect_actions_quote
    ...
    services_view_store = mongo_factory.create('services', VIEW_STORE)
    ...
    updated_quote = quote_repo.find("_id" => quote[:quote_reference]).to_a[0]
    assert_equal("actioned", updated_quote["status"])
    assert_equal("27/06/2014", updated_quote["actioned_on"])
